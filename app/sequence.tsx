...
type SequenceSessionWithPlayers = {
  id: string;
  created_at: string;
...
export default function SequenceSessionsScreen() {
  const { newSession } = useLocalSearchParams<{ newSession?: string }>();
  const [activeSessions, setActiveSessions] = useState<SequenceSessionWithPlayers[]>([]);
  ...
  const renderSession = ({ item: session }: { item: SequenceSessionWithPlayers }) => {
    ...
    return (
      <Pressable
        ...
        onPress={() => router.push(`/sequence-session/${session.id}`)}>
        ...
        {Object.entries(playersByTeam).map(([team, players]) => {
          ...
          return (
            <View key={team} style={styles.teamSection}>
              <View style={[styles.teamBadge, { backgroundColor: team === 'blue' ? '#1E88E5' : team === 'green' ? '#43A047' : '#E53935' }]}>
                <Text style={styles.teamName}>{team.charAt(0).toUpperCase() + team.slice(1)} Team</Text>
              </View>
              {players.map(player => (
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Link href="/new-sequence-session?redirect=true" asChild>
          <Pressable style={styles.newSessionButton}>
            <MaterialCommunityIcons name="plus" size={24} color="#fff" />
            <Text style={styles.newSessionText}>New Sequence Game</Text>
          </Pressable>
        </Link>
        <Pressable 
          style={({ pressed }) => [
      ...
      {showNewSessionNotification && (
        <View style={styles.notificationBanner}>
          ...
          <Text style={styles.notificationText}>
            New sequence game created! Pull down to refresh and see active games.
          </Text>
          <Pressable 
      ...
      {!loading && activeSessions.length === 0 ? (
        <View style={styles.emptyState}>
          <MaterialCommunityIcons name="cards-playing-outline" size={64} color="#FF5A5F" />
          <Text style={styles.emptyStateTitle}>No Active Sequence Games</Text>
          <Text style={styles.emptyStateText}>
            Start a new sequence game to begin tracking scores and winners
          </Text>
        </View>
      ) : (
        <FlatList
