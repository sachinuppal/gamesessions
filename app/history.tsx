...
export default function HistoryScreen() {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'poker' | 'rummy'>('all');

  const loadCompletedSessions = useCallback(async () => {
    try {
      setError(null);
      
      // Check for settlements that need to be marked as settled by goons
      ...
      // Combine and sort all sessions by ended_at
    ...
    <View style={styles.container}>
      ...
      {filteredSessions.length === 0 && !loading ? (
        <View style={styles.emptyState}>
          <MaterialCommunityIcons name="history" size={64} color="#FF5A5F" />
