...
type RummySessionWithPlayers = Session & {
  players: Player[];
  game_type?: string;
  player_count?: number;
  entry_fee?: number;
...
export default function RummySessionsScreen() {
  const { newSession } = useLocalSearchParams<{ newSession?: string }>();
  const [activeSessions, setActiveSessions] = useState<RummySessionWithPlayers[]>([]);
  ...
  const renderSession = ({ item: session }: { item: RummySessionWithPlayers }) => {
    const totalEntryFee = session.players.length * (session.entry_fee || 0);
    const isOwner = sessionOwnership[session.id];
    const gameTypeDisplay = session.game_type === 'pool_101' ? 'Pool 101' : 'Pool 201';

    return (
      <Pressable
        style={styles.sessionCard}
        onPress={() => router.push(`/rummy-session/${session.id}`)}>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Link href="/new-rummy-session?redirect=true" asChild>
          <Pressable style={styles.newSessionButton}>
            <MaterialCommunityIcons name="plus" size={24} color="#fff" />
            <Text style={styles.newSessionText}>New Rummy Game</Text>
          </Pressable>
        </Link>
        <Pressable 
          style={({ pressed }) => [
      ...
      {showNewSessionNotification && (
        <View style={styles.notificationBanner}>
          ...
          <Text style={styles.notificationText}>
            New rummy game created! Pull down to refresh and see active games.
          </Text>
          <Pressable 
      ...
      {!loading && activeSessions.length === 0 ? (
        <View style={styles.emptyState}>
          <MaterialCommunityIcons name="cards" size={64} color="#FF5A5F" />
          <Text style={styles.emptyStateTitle}>No Active Rummy Games</Text>
          <Text style={styles.emptyStateText}>
            Start a new rummy game to begin tracking scores and winners
          </Text>
        </View>
      ) : (
        <FlatList
