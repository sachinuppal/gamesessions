...
export default function PokerSessionsScreen() {
  const { newSession } = useLocalSearchParams<{ newSession?: string }>();
  const [activeSessions, setActiveSessions] = useState<SessionWithPlayers[]>([]);
  ...
  const renderSession = ({ item: session }: { item: SessionWithPlayers }) => {
        ...
        <View style={styles.playerList}>
          {session.players.map((player) => (
            <View key={player.id} style={styles.playerRow}>
              ...
              <Text style={styles.buyIn}>₹{player.total_buy_in}</Text>
            </View>
          ))}
        </View>

        <View style={styles.sessionFooter}>
          <Text style={styles.totalLabel}>Total Buy-in</Text>
          <Text style={styles.totalAmount}>₹{totalBuyIn}</Text>
        </View>
      </Pressable>
    );
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Link href="/new-poker-session?redirect=true" asChild>
          <Pressable style={styles.newSessionButton}>
            <MaterialCommunityIcons name="plus" size={24} color="#fff" />
            <Text style={styles.newSessionText}>New Poker Session</Text>
          </Pressable>
        </Link>
        <Pressable 
          style={({ pressed }) => [
      ...
      {showNewSessionNotification && (
        <View style={styles.notificationBanner}>
          ...
          <Text style={styles.notificationText}>
            New session created! Pull down to refresh and see active sessions.
          </Text>
          <Pressable 
      ...
      {!loading && activeSessions.length === 0 ? (
        <View style={styles.emptyState}>
          <MaterialCommunityIcons name="cards-playing" size={64} color="#FF5A5F" />
          <Text style={styles.emptyStateTitle}>No Active Poker Sessions</Text>
          <Text style={styles.emptyStateText}>
            Start a new poker session to begin tracking chips and settlements
          </Text>
        </View>
      ) : (
        <FlatList
